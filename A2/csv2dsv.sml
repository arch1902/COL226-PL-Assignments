...
exception LastRecordNotTerminatedbyNewline ;
  ...
  let
    ...
    fun helper(prev : char, ch : char option ,cnt : int,prevcnt :int,linenum :int, bt :int , field :string , size : int , dq :int) =
      case ch of
           NONE => 
                  ( 
                   if size=0 then (TextIO.closeIn ins; TextIO.closeOut outs)
                   else raise LastRecordNotTerminatedbyNewline
                  )
         ...
         | SOME(c) => 
            ...
            else if c = delim2 then (helper(c,TextIO.input1 ins,cnt,prevcnt,linenum,bt,field^String.str(c),size+1,1))
...
convertDelimiters("test2.txt",#",","t1.txt",#";");
